 ...
 */
class inmobile_sms_handler extends CRM_SMS_Provider {
  ...
  function __construct($provider, $skipAuth = TRUE) {
    ...
    $this->_api = new InmobileApi($provider['password']);
  }
  ...
  static function &singleton($providerParams = array(), $force = FALSE) {
    ...
    if (!isset(self::$_singleton[$cacheKey]) || $force) {
      ...
      self::$_singleton[$cacheKey] = new inmobile_sms_handler($provider, $skipAuth);
    }
  ...
  function send($recipients, $header, $message, $jobID = NULL, $userID = NULL) {
    $id = date('YmdHis');
    try {
        $response = $this->_api->messages()->send(
